...
const defaultChromeIphoneEmulationDeviceName = 'iPhone 6';
  ...
  async analyzeUrl(url, options, pluginScripts, pluginAsyncScripts) {
    ...
    if (options.mobile) {
      ...
      if (btOptions.browser === 'chrome') {
        ...
        if (!'deviceName' in btOptions.chrome.mobileEmulation &&
            !'userAgent' in btOptions.chrome.mobileEmulation) {
          btOptions.chrome.mobileEmulation.deviceName = defaultChromeIphoneEmulationDeviceName;
        }
      } else {
